const CONFIG = {
    ...
    // Si tu ouvres index.html depuis ton PC en local, l'API sera appelée sur l'ESP32.
    // Si plus tard la page est servie directement par l'ESP32, cette valeur fonctionnera aussi.
...
function setupActions() {
    ...
    if (elements.turnInPlaceBtn) {
        elements.turnInPlaceBtn.addEventListener("click", startTurnInPlace);
    }
...
async function startCircleSequence() {
    ...
    try {
        await apiGet(`${CONFIG.circleSequenceEndpoint}?radius=${encodeURIComponent(radius)}&direction=${encodeURIComponent(direction)}`);
        addLog(`Sequence cercle lancee R=${radius} cm`);
    } catch (error) {
        addLog("Erreur lancement cercle");
